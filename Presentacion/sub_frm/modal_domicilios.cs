...
{
    public partial class modal_domicilios : Form
    {
        D_domicilios domicilios = new D_domicilios();
        private int? id;
        public modal_domicilios(int? id = null)
        {
            InitializeComponent();
            this.id = id;
        }
        ...
        private void modal_domicilios_Load(object sender, EventArgs e)
        {
            if (id != null)
            {
                DataTable tabla = new DataTable();
                tabla = domicilios.Consultarxid((int)id);
                string nom = tabla.Rows[0]["zona"].ToString();
